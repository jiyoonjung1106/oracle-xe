...
-- RPAD() - 길이가 n이 되도록 왼쪽부터 문자식으로 채운 표현식을 반환합니다.
SELECT RPAD('salary', 10, '*') FROM dual;

-- REPLACE() - 문자열 치환
SELECT REPLACE('JACK and JUE', 'J', 'BL') FROM dual;

-- TRIM() - 문자열에서 선행문자나 후행 문자를 자릅니다.
...
TO_CHAR() 함수 - 날짜 또는 숫자 사용가능
   ...
   TIMESTAMP 타입
   FF   - 밀리세컨드(뒤에 숫자써서 몇자리까지 오게할지 정할 수 있음
*/
...
/*
NULLIF() 함수
     두 표현식을 비교 같으면 null 반환, 다르면 expr1을 반환합니다.
...
SELECT job_id, salary,
  CASE job_id WHEN 'IT_PROG' THEN salary+100
        WHEN 'FT_ACCOUNT' THEN salary+100
      ELSE salary END "REVISED_SALARY"
FROM employees;
